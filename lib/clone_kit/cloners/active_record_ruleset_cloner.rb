  ...
  module Cloners
    class ActiveRecordRulesetCloner
      ...
      def insert(record)
        ...
        with_connection(current_operation.arguments["write_postgres_url"]) do
      ...
      def each_existing_record(ids)
        ids.each do |id|
          record = with_connection(current_operation.arguments["read_postgres_url"]) do
            model_klass.find_by(model_klass.primary_key => id).attributes
          end
